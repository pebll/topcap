...
TEST_CASE("coordsToTile converts correctly", "[utils]") {
  REQUIRE(utils::coordsToTile(Coordinates{0, 0}) == "a1");
  REQUIRE(utils::coordsToTile(Coordinates{0, 1}) == "a2");
  REQUIRE(utils::coordsToTile(Coordinates{1, 0}) == "b1");
  REQUIRE(utils::coordsToTile(Coordinates{3, 3}) == "d4");
  REQUIRE(utils::coordsToTile(Coordinates{5, 5}) == "f6");
}
...
TEST_CASE("tileToCoords and coordsToTile are inverse", "[utils]") {
  ...
  for (const auto &tile : tiles) {
    ...
    std::string back = utils::coordsToTile(coords);
    REQUIRE(back == tile);
  }
}
